...
resource "null_resource" "aws_basicconfig" {
  count = "${var.aws_instance_count}"
  connection {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sed -i.repl -e 's/#NODENAME#/awsconsuldemo${count.index}/g' -e 's/#PUBLICIP#/${element(aws_instance.consuldemo.*.public_ip,count.index)}/g' -e 's/#DC#/ConsulDemo/g' -e 's/#JOINNODES#/${join("\\\",\\\"",concat(aws_instance.consuldemo.*.public_ip,cloudstack_ipaddress.consuldemo.*.ipaddress))}/g' C:\consul\config\config.json",
...
resource "null_resource" "aws_orekconfig" {
  count = "${var.aws_instance_count}"
  depends_on = "null_resource.aws_basicconfig"
  connection {
    host= "${element(aws_instance.consuldemo.*.public_ip, count.index)}"
    type = "winrm"
    timeout = "30m"
